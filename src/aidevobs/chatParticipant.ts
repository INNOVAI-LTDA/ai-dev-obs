...
import { AiDevObsSessionManager } from './sessionManager';
import { AiDevObsEventStore } from './eventStore';
import { generateModuleReport, generateSummary } from './summaryGenerator';
import { AiDevObsModelService } from './modelService';
...
export function registerAiDevObsChatParticipant(
  context: vscode.ExtensionContext,
  sessionManager: AiDevObsSessionManager,
  store: AiDevObsEventStore
): vscode.Disposable {
  const modelService = new AiDevObsModelService(context);
  ...
  const handler: vscode.ChatRequestHandler = async (
    ...
    try {
      ...
      if (prompt.startsWith('/report')) {
        ...
        const events = await store.readEvents(session.sessionId);
        const report = generateModuleReport(events, moduleId);
      ...
      try {
        ...
        await sessionManager.logAssistantResponse(
          'AI model response captured by AIDevObs.',
          result.text,
          userEventId,
          {
            model: result.model,
        ...
        await sessionManager.logAssistantResponse(
          'Model call failed; AIDevObs returned logger-only fallback.',
          fallbackResponse,
          userEventId,
          {
            modelError: message,
