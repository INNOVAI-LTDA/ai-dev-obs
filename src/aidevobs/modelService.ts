...
import { AiDevObsModelInfo, AiDevObsModelObservability } from './types';

const SELECTED_MODEL_KEY = 'obs.selectedLanguageModel';

export interface AiDevObsModelServiceResult {
  text: string;
  model?: AiDevObsModelInfo;
  observability: AiDevObsModelObservability;
}

export interface AiDevObsAvailableModelInfo extends AiDevObsModelInfo {
  index: number;
  maxInputTokens?: number;
  raw?: Record<string, unknown>;
}

export interface AiDevObsSelectedModelInfo extends AiDevObsModelInfo {
  index?: number;
  maxInputTokens?: number;
  selectedAt: string;
}

export class AiDevObsModelService {
  constructor(private readonly context: vscode.ExtensionContext) {}

  async listModels(): Promise<AiDevObsAvailableModelInfo[]> {
    const models = await vscode.lm.selectChatModels({});

    return models.map((model, index) => this.toAvailableModelInfo(model, index));
  }

  getSelectedModel(): AiDevObsSelectedModelInfo | undefined {
    return this.context.workspaceState.get<AiDevObsSelectedModelInfo>(SELECTED_MODEL_KEY);
  }
  ...
  async selectModel(identifier: string): Promise<AiDevObsSelectedModelInfo> {
    ...
    const selectedModel: AiDevObsSelectedModelInfo = {
      id: selected.id,
      name: selected.name,
  ...
  formatModelsAsMarkdown(models: AiDevObsAvailableModelInfo[]): string {
  ...
  formatSelectionResultAsMarkdown(model: AiDevObsSelectedModelInfo): string {
    return [
  ...
  async ask(prompt: string, stream: vscode.ChatResponseStream, token: vscode.CancellationToken): Promise<AiDevObsModelServiceResult> {
  ...
  private toAvailableModelInfo(model: vscode.LanguageModelChat, index: number): AiDevObsAvailableModelInfo {
    const modelRecord = model as unknown as Record<string, unknown>;
  ...
  private findModelByIdentifier(models: AiDevObsAvailableModelInfo[], identifier: string): AiDevObsAvailableModelInfo | undefined {
  ...
  private isSameModel(model: AiDevObsModelInfo | undefined, selected: AiDevObsModelInfo | undefined): boolean {
    if (!model || !selected) {
      return false;
    }
