# AIDevObs Conversation Trace - Architecture Notes

## Core idea

AIDevObs is a local repository layer plus a VS Code extension experience.

In v0.2.2, AIDevObs is an observable AI proxy with explicit model selection:
...
This MVP captures only interactions routed through `@obs`.
...
The selection is stored in VS Code `workspaceState`, not in `.bx/obs.yaml`. This keeps local provider/model preference out of Git while preserving session evidence in `.bx/`.
...
@obs user prompt
  ...
  -> obs.model.request.started
  -> obs.model.request.failed
  -> obs.chat.assistant_response with mode logger_only_fallback
```
...
```text
src/aidevobs/chatParticipant.ts
```

Owns `@obs`, routes commands, logs prompts, calls the model service and streams output.

```text
src/aidevobs/modelService.ts
```
